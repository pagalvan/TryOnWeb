...
INSERT INTO public.tryon_items (id, session_id, prenda_id, estado, duracion_seg, feedback)
SELECT gen_random_uuid(), c.session_id, c.prenda_id,
       (ARRAY['exito','parcial','descartado'])[(random()*3)::int + 1],
       45 + (random()*90)::int,
       jsonb_build_object('seed','demo-dashboard','satisfaccion', round((random()*5)::numeric,2))
